...
export class LogicService {
  ...
  getRandomQuestions() {
    ...
    return this.apiService.getRandomQuestionsFromAPI().pipe(
      map((data: any) => {
        return data.results.map((trivia: any) => {
          const formattedAnswers = trivia.incorrect_answers.map(
            (answer: string) => decodeHtmlEntities(answer)
          );
          return {
            question: decodeHtmlEntities(trivia.question),
            options: [
              decodeHtmlEntities(trivia.correct_answer),
              ...formattedAnswers,
            ],
            correctOption: trivia.correct_answer,
          };
        });
      })
    );
